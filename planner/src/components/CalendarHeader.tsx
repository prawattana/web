...
export default function CalendarHeader({ title, view, strip, onPrev, onNext, onToday, onView, onPickDay }: Props) {
  return (
    <div className="px-2.5 pt-1.5">
      {/* แถวบน: นำทาง + ชื่อ + วันนี้ + สลับมุมมอง (รวมแถวเดียวให้กระชับ) */}
      <div className="mb-1.5 flex items-center gap-1.5">
        <button onClick={onPrev} className="btn-glass flex h-7 w-7 items-center justify-center rounded-full text-base">‹</button>
        <h2 className="flex-1 truncate text-center text-[13px] font-bold tracking-tight text-white">{title}</h2>
        <button onClick={onNext} className="btn-glass flex h-7 w-7 items-center justify-center rounded-full text-base">›</button>
        <button onClick={onToday} className="btn-glass rounded-full px-2.5 py-1 text-[11px] font-semibold">วันนี้</button>
      </div>

      {/* segmented control */}
      <div className="mb-1.5 flex gap-0.5 rounded-xl bg-white/5 p-0.5">
        {VIEWS.map((v) => (
